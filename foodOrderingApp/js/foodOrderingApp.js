...
class OrderFood {
    ...
    setRecursiveOnItems(allItems, value) {        
        ...
        //     } else if(item.menu) {
        //         return this.setRecursiveOnItems(item.menu,value,filteredData);
        //     }
        // })
    ...
    createContainer(items, width,height) {
        ...
        items.forEach((item) => {
            views +=`<div class="grid-item list-box" id="${item.id}">
                ...
                <span class="inline" data-parent-id="${item.id}">
                    <div data-parent-id="${item.id}" class="heading"><strong>${item.name}</strong>
                    <span>${item.tags[0]},${item.tags[1]}</span>
                    </div>
                    <div data-parent-id="${item.id}" class="rating ${parseInt(item.rating) > 3.5 ? 'green' : parseInt(item.rating) < 2 ? 'red' : 'yellow'}">${item.rating}<i class="fa fa-star checked"></i></div>
                </span>
            </div>`
        })
        return views;
    ...
    createSortContainerView() {
        const sortList = ["Popularity","Rating: High to Low","Delivery Time","Cost: Low to High","Cost: High to Low"];
        let view = "";
    ...
    handleCheckboxBtn(event){
        ...
        if(this.filters.cuisines !== undefined) {
            if(this.filters.cuisines.indexOf(event.target.value) === -1) {
                this.filters.cuisines.push(event.target.value);
            }
        } else if(this.filters.cuisines === undefined) {
            this.filters.cuisines = [].concat(event.target.value);
        }
